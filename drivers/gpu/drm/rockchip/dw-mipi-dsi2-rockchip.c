...
static int ff_dw_mipi_dsi2_encoder_mode_set(struct dw_mipi_dsi2 *dsi2)
{
...
static void ff_dw_mipi_dsi2_encoder_atomic_disable(struct dw_mipi_dsi2 *dsi2)
{
	if (dsi2->panel)
		drm_panel_disable(dsi2->panel);
			    ...
			    void *data)
{
	...
	if(uboot_check_state == FF_CHECK || uboot_check_state == FF_NONE){
		...
		if(dsi2->firefly_check) {
			ret = ff_dw_mipi_dsi2_encoder_mode_set(dsi2);
			ff_dw_mipi_dsi2_encoder_atomic_disable(dsi2);
			if(ret)
				return ret;
		}
	}
