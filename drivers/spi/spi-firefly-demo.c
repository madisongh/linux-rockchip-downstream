...
static int firefly_spi_read_xm25x_id_0(struct spi_device *spi)
{       
        int     status;
        ...
        printk("%s ID = %02x %02x %02x\n", __FUNCTION__, rbuf[0], rbuf[1], rbuf[2]);
        return status;
}


static int firefly_spi_read_xm25x_id_1(struct spi_device *spi)
{
        int     status;
        char tbuf[] = {FIREFLY_SPI_READ_ID_CMD};
        char rbuf[3];
...
static int firefly_spi_probe(struct spi_device *spi)
{
    ...
    printk("firefly spi demo\r\n");
        ...
        printk("spi demo mode ; %d \n", (int) (spi->mode));

        firefly_spi_read_xm25x_id_0(spi);
        firefly_spi_read_xm25x_id_1(spi);
