...
{
	gpiod_set_value_cansleep(wk2124_dev->cs, !!on);
}
...
static int wk2xxx_probe(struct spi_device *spi)
{
    ...
    wk2xxx_dev->pwr = devm_gpiod_get(&spi->dev, "power", GPIOD_OUT_HIGH);
    if (IS_ERR(wk2xxx_dev->pwr)) {
        printk("%s() devm_gpio_request pwr_gpio request ERROR\n", __FUNCTION__);
        //goto err;
    }

    wk2xxx_dev->rst = devm_gpiod_get(&spi->dev, "reset", GPIOD_OUT_HIGH);
    if (IS_ERR(wk2xxx_dev->rst)) {
        printk("%s() devm_gpio_request rst_gpio request ERROR\n", __FUNCTION__);
        goto err;
    }
    msleep(50);
    gpiod_set_value_cansleep(wk2xxx_dev->rst, 0);
    msleep(100);
    gpiod_set_value_cansleep(wk2xxx_dev->rst, 1);
    msleep(20);
    ...
    wk2xxx_dev->cs = devm_gpiod_get(&spi->dev, "cs", GPIOD_OUT_LOW);
    if (IS_ERR(wk2xxx_dev->cs)) {
