...
static void himax_mcu_read_FW_ver(void)
{
	...
	ic_data->vendor_config_ver = data[4] << 8 | data[5];
		...
		int length)
{
	...
	tmp_data[3] = (FW_PAGE_SEZE / 4 - 1) >> 8; /*access_unit*/
	tmp_data[2] = FW_PAGE_SEZE / 4 - 1; /*access_unit*/
	tmp_data[1] = on_pflash_op->data_auto[0];/*0xA5;*/
	tmp_data[0] = on_pflash_op->data_main_read[0];/*0x03;*/
	g_core_fp.fp_register_write(on_pflash_op->addr_ctrl_auto,
			DATA_LEN_4, tmp_data, 0);
	...
	addr = addr + FW_PAGE_SEZE) {
		I("%s: addr = %X, i = %d\n", __func__, addr, i);
		tmp_addr[0] = (uint8_t)addr;
		tmp_addr[1] = (uint8_t)(addr >> 8);
		tmp_addr[2] = (uint8_t)(addr >> 16);
		tmp_addr[3] = (uint8_t)(addr >> 24);
		g_core_fp.fp_register_read(tmp_addr, FW_PAGE_SEZE, tmp_data, 0);
		memcpy(&r_buf[i], tmp_data, FW_PAGE_SEZE * sizeof(uint8_t));
		i = i + FW_PAGE_SEZE;
	}
	I("%s: Finished!\n", __func__);
}
...
static int himax_mcu_fw_ver_bin(void)
{
	...
	if (i_CTPM_FW != NULL) {
		...
		g_i_CFG_VER = (i_CTPM_FW[CFG_VER_MAJ_FLASH_ADDR] << 8) | 0x5A;
...
static uint8_t himax_mcu_tp_info_check(void)
{
	...
	uint16_t x_res = on_pdriver_op->data_df_x_res[1] << 8
			| on_pdriver_op->data_df_x_res[0];
	uint16_t y_res = on_pdriver_op->data_df_y_res[1] << 8
			| on_pdriver_op->data_df_y_res[0];
	uint8_t err_cnt = 0;
	...
	if (ic_data->HX_Y_RES < (y_res / 8)
	|| ic_data->HX_Y_RES > (y_res * 3 / 2)) {
		ic_data->HX_Y_RES = y_res;
		err_cnt |= 0x08;
	}

	if (ic_data->HX_X_RES < (x_res / 8)
...
static void himax_mcu_touch_information(void)
{
	...
	/*cmd[3] = 0x08; cmd[2] = 0x00; cmd[1] = 0x00; cmd[0] = 0x30;*/
	g_core_fp.fp_register_read(on_pdriver_op->addr_fw_define_x_y_res,
			DATA_LEN_4, data, 0);
	ic_data->HX_Y_RES = (data[0] << 8) + data[1];
	ic_data->HX_X_RES = (data[2] << 8) + data[3];
	...
	ic_data->HX_Y_RES = (data[1] << 8) + data[0];
	ic_data->HX_X_RES = (data[3] << 8) + data[2];
...
/* CORE_DRIVER */
	...
	himax_parse_assign_cmd(on_driver_data_df_x_res,
		on_pdriver_op->data_df_x_res,
		sizeof(on_pdriver_op->data_df_x_res));
	himax_parse_assign_cmd(on_driver_data_df_y_res,
		on_pdriver_op->data_df_y_res,
		sizeof(on_pdriver_op->data_df_y_res));
/* CORE_DRIVER */
}
