...
static DEVICE_ATTR(firefly_pwm, (S_IWUSR | S_IRUSR | S_IWGRP | S_IRGRP),
		   firefly_pwm_show, firefly_pwm_store);
#endif
...
ssize_t firefly_pwm_parse_dt(struct firefly_pwm_data *firefly_pdata,
...
static int firefly_pwm_probe(struct platform_device *pdev)
{
	...
	printk("firefly pwm probe start \n");
	if (!np) {
	...
	firefly_pdata =
		devm_kzalloc(&pdev->dev, sizeof(*firefly_pdata), GFP_KERNEL);
	if (!firefly_pdata)
		return -ENOMEM;
	...
	printk("firefly pwm firefly_pdata->pwm_period_ns : %d \n",
	       firefly_pdata->pwm_period_ns);
	if (firefly_pdata->pwm_period_ns > 0)
		pwm_set_period(firefly_pdata->pwm,
			       firefly_pdata->pwm_period_ns);

	firefly_pdata->period = pwm_get_period(firefly_pdata->pwm);
	printk("firefly pwm firefly_pdata->period : %d \n",
	       firefly_pdata->period);
